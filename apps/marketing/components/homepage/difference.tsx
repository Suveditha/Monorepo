'use client';

import * as React from 'react';
import { Check, Play, VolumeOff, X } from 'lucide-react';

import { cn } from '@workspace/ui/lib/utils';

import { Badge } from './components/badge';

export default function Difference(): React.JSX.Element {
  const [activeVideo, setActiveVideo] = React.useState<number | null>(null);
  const [pausedVideos, setPausedVideos] = React.useState<
    Record<number, boolean>
  >({});
  const videoRefs = React.useRef<HTMLVideoElement[]>([]);

  const handlePlayPause = React.useCallback(
    (index: number) => {
      const selected = videoRefs.current[index];
      if (!selected) return;

      // ✅ If same video and playing → pause it
      if (activeVideo === index && !selected.paused) {
        selected.pause();
        setPausedVideos((prev) => ({ ...prev, [index]: true }));
        selected.controls = false;
        return;
      }

      // ✅ Pause all other videos
      videoRefs.current.forEach((vid, i) => {
        if (vid && i !== index) {
          vid.pause();
          vid.currentTime = 0;
          vid.controls = false;
          setPausedVideos((prev) => ({ ...prev, [i]: true }));
        }
      });

      // ✅ Play clicked video instantly
      selected.muted = false;
      selected.controls = true;

      const playPromise = selected.play();
      if (playPromise !== undefined) {
        playPromise
          .then(() => {
            setActiveVideo(index);
            setPausedVideos((prev) => ({ ...prev, [index]: false }));
          })
          .catch((err) => {
            console.warn('Autoplay blocked:', err);
          });
      }

      // ✅ Reset when ended
      selected.onended = () => {
        selected.controls = false;
        setActiveVideo(null);
        setPausedVideos((prev) => ({ ...prev, [index]: true }));
      };
    },
    [activeVideo]
  );

  const videos = [
    {
      state: 'Before VideoMule',
      color: 'bg-red-50 text-red-600',
      src: 'https://gw-ai-social-media.b-cdn.net/VideoMule/Slient.mp4',
      checkMark: X,
      title: 'Silent Screen Recording',
      points: [
        {
          heading: 'No Audio',
          description: "Your input video doesn't need to have any audio"
        },
        {
          heading: 'Low Engagement',
          description: 'Viewers lose interest without audio guidance'
        },
        {
          heading: 'Manual Script & Voiceover takes time',
          description: 'Expensive voice talent and studio time'
        }
      ]
    },
    {
      state: 'After VideoMule',
      color: 'bg-videomule-green/10 text-videomule-green',
      src: 'https://gw-ai-social-media.b-cdn.net/VideoMule/audio.mp4',
      checkMark: Check,
      title: 'Professional AI Voiceover',
      points: [
        {
          heading: 'Automatic Audio + Sync',
          description: 'AI script perfectly explains every step'
        },
        {
          heading: '5x Higher Engagement',
          description: 'Professional narration keeps viewers watching'
        },
        {
          heading: 'Ready in Minutes',
          description: 'AI prepares your output in minutes'
        }
      ]
    }
  ];

  return (
    <section className="py-28 bg-gradient-header relative overflow-hidden">
      {/* Decorative Elements */}
      <div className="absolute top-20 left-20 w-32 h-32 bg-videomule-green/5 rounded-full blur-3xl"></div>
      <div className="absolute bottom-20 right-20 w-40 h-40 bg-blue-400/5 rounded-full blur-3xl"></div>

      <div className="max-w-7xl mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-15">
          <Badge
            variant="secondary"
            className="mb-6 text-videomule-green bg-videomule-green/10 border-videomule-green/20"
          >
            See the Difference
          </Badge>
          <h2 className="font-bricolage text-4xl lg:text-6xl font-bold text-videomule-black mb-4">
            Before vs After VideoMule
          </h2>
          <p className="text-xl text-videomule-green-text max-w-3xl mx-auto leading-relaxed">
            Transform your silent screen recordings into professional, engaging
            content that captivates your audience
          </p>
        </div>

        {/* Comparison Grid */}
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {videos.map((video, index) => {
            const isPaused = pausedVideos[index];
            const isActive = activeVideo === index && !isPaused;

            return (
              <div
                key={video.state}
                className="space-y-8 p-8 border border-[#DBEAFE] rounded-2xl bg-videomule-white"
              >
                <div className="text-center">
                  <div
                    className={cn(
                      video.color,
                      'inline-flex items-center rounded-full px-5 py-2'
                    )}
                  >
                    <video.checkMark className="w-5 h-5 mr-2" />
                    <span className="font-semibold text-sm">{video.state}</span>
                  </div>
                </div>

                {/* Video */}
                <div className="bg-videomule-white rounded-2xl shadow-lg border border-videomule-gray/15 overflow-hidden">
                  <div
                    className={cn(
                      index === 1
                        ? 'bg-videomule-green/5 border-b border-videomule-green/20'
                        : 'bg-videomule-gray/10 border-b',
                      'px-4 py-3 flex items-center'
                    )}
                  >
                    <div className="flex space-x-2">
                      <div className="w-3 h-3 bg-red-400 rounded-full"></div>
                      <div className="w-3 h-3 bg-yellow-400 rounded-full"></div>
                      <div
                        className={cn(
                          index === 1 ? 'bg-videomule-green' : 'bg-green-400',
                          'w-3 h-3 rounded-full'
                        )}
                      ></div>
                    </div>
                    <div className="flex-1 text-center">
                      <span
                        className={cn(
                          index === 1
                            ? 'text-videomule-green font-medium'
                            : 'text-videomule-gray',
                          'text-sm'
                        )}
                      >
                        {video.title}
                      </span>
                    </div>
                  </div>

                  {/* Bunny Video Frame */}
                  <div className="aspect-video flex items-center justify-center relative bg-gradient-to-br from-gray-200 to-gray-300">
                    <video
                      ref={(el) => {
                        if (el) videoRefs.current[index] = el;
                      }}
                      src={video.src}
                      className="absolute inset-0 w-full h-full object-cover cursor-pointer"
                      controls={isActive}
                      preload="metadata"
                      playsInline
                    />

                    {/* Custom Play Button (Click directly to play) */}
                    {(!isActive || isPaused) && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handlePlayPause(index);
                        }}
                        className="absolute inset-0 flex items-center justify-center"
                      >
                        <span className="bg-white text-videomule-green rounded-full w-12 h-12 flex items-center justify-center shadow-md hover:scale-105 transition-transform">
                          <Play className="w-8 h-8" />
                        </span>
                      </button>
                    )}

                    {/* Indicators */}
                    {index === 0 ? (
                      <div className="absolute top-4 right-4 bg-red-500 rounded-full p-2">
                        <X className="w-4 h-4 text-white" />
                      </div>
                    ) : (
                      <div className="absolute top-4 right-4 bg-videomule-green rounded-full p-2">
                        <div className="flex items-center space-x-1">
                          <div className="w-1 h-2 bg-white rounded-full animate-pulse"></div>
                          <div
                            className="w-1 h-4 bg-white rounded-full animate-pulse"
                            style={{ animationDelay: '0.1s' }}
                          ></div>
                          <div
                            className="w-1 h-3 bg-white rounded-full animate-pulse"
                            style={{ animationDelay: '0.2s' }}
                          ></div>
                          <div
                            className="w-1 h-5 bg-white rounded-full animate-pulse"
                            style={{ animationDelay: '0.3s' }}
                          ></div>
                        </div>
                      </div>
                    )}
                    {index === 0 ? (
                      <div className="absolute top-4 left-4 bg-black rounded-full p-2">
                        <VolumeOff className="w-4 h-4 text-white font-bold" />
                      </div>
                    ) : (
                      ''
                    )}
                  </div>
                </div>

                {/* Points */}
                <div className="space-y-5">
                  {video.points.map((point) => (
                    <div
                      key={point.heading}
                      className="flex items-start"
                    >
                      <div
                        className={cn(
                          video.color,
                          'w-6 h-6 rounded-full flex items-center justify-center mr-4 mt-1'
                        )}
                      >
                        <video.checkMark className="w-4 h-4" />
                      </div>
                      <div>
                        <h4 className="font-semibold text-lg text-videomule-black mb-1">
                          {point.heading}
                        </h4>
                        <p className="text-videomule-gray text-base">
                          {point.description}
                        </p>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            );
          })}
        </div>
      </div>

      {/* Section separator */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gray-300 to-transparent"></div>
    </section>
  );
}
